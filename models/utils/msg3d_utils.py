...
class ST_MSGCN(nn.Module):
    def __init__(self,
        ...
        A_scales_spatial = [k_adjacency(A + np.eye(len(A)), k, with_self=True) for k in range(num_scales)]
        A_scales_spatial = [normalize_digraph(g) for g in A_scales_spatial]
        A_scales_st = [np.tile(g, (window_size, 1)).copy() for g in A_scales_spatial]
        A_scales = np.stack(A_scales_st)
    ...
    def build_st_graph(self, A, window_size):
        ...
        A_large = np.tile(A_with_I, (window_size, 1)).copy()
        
        return A_large
    
    def forward(self, x):
        N, C, T, V = x.shape    # T = number of windows, V = self.V * window_size -> (N, C, T, V*window_size)
        A = self.A + self.PA # (K, V*window_size, V*window_size)

        # Perform Graph Convolution
        res = self.residual(x)
        agg = torch.einsum('kvu,nctv->nkctu', A, x) # (K, V*window_size, V*window_size) , (N, C, T, V*window_size) -> (N, K, C, T, V*window_size)
        agg = agg.reshape(N, self.num_scales * C, T, V//self.window_size)
        out = self.mlp(agg)
        out_res = out + res
...
class MSG3DBlock(nn.Module):
    def __init__(self,
        ...
        if embed_factor == 1:
            ...
            if False: #in_channels == 4: # 첫번재 블럭일경우 임베딩 채널 아웃풋이 아웃채널과 같아야하는 기본 조건이었는데, 실험해보니 성능이 더 안좋았음
        ...
        self.out_conv = nn.Conv2d(self.embed_channels_out, out_channels, kernel_size=1)
